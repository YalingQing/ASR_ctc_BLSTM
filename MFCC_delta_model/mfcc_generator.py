...
def read_wav(wav_path, nfilt):
    ...
    coef = mfcc(audio, samplerate=samplerate, nfilt = nfilt, numcep = nfilt)
    first_delta = delta(coef, 1)
    second_delta = delta(first_delta, 3)
    x = np.append(coef, first_delta, axis = 1)
    x = np.append(x, second_delta, axis = 1)
...
def pad_wav(wavs_path, nfilt):
    ...
    X = np.ones((audio_num, max_length, nfilt*3)) * (-1)
